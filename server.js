// server.js (with corrected session initialization)
...
async function startServer() {
  try {
    ...
    app.use(session({
      secret: APP_SECRET,
      resave: false,
      saveUninitialized: true, // THIS IS THE FIX: Force session to be saved for OIDC
      cookie: {
        secure: true,
        httpOnly: true,
        sameSite: 'none'
    ...
    app.get('/ui/groups', oidc.ensureAuthenticated(), async (req, res) => {
        try {
            const query = `
                ...
                FROM groups g LEFT JOIN group_members gm ON g.id = gm.group_id LEFT JOIN users u ON gm.user_id = u.id
