...
const USER_RESOURCE_TYPE = { "schemas": ["urn:ietf:params:scim:schemas:core:2.0:ResourceType"], "id": "User", "name": "User", "endpoint": "/scim/v2/Users", "description": "User Account", "schema": "urn:ietf:params:scim:schemas:core:2.0:User", "schemaExtensions": [{ "schema": "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User", "required": false }], "meta": { "location": "/scim/v2/ResourceTypes/User", "resourceType": "ResourceType" } };
...
async function startServer() {
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS groups (id TEXT PRIMARY KEY, displayName TEXT, scim_data JSONB)`);
    await pool.query(`ALTER TABLE groups DROP CONSTRAINT IF EXISTS groups_displayname_key`);
    ...
    app.get('/ui/users', oidc.ensureAuthenticated(), async (req, res) => {
        try {
            ...
            const users = rows.map(row => row.scim_data).filter(u => u && u.userName);
