// server.js (with new /ui/groups route)
...
pool.query('SELECT NOW()', (err, res) => {
    if (err) { console.error("Error connecting to the database:", err.message); }
        ...
        pool.query(`CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, userName TEXT UNIQUE, active BOOLEAN, scim_data JSONB)`);
        pool.query(`CREATE TABLE IF NOT EXISTS groups (id TEXT PRIMARY KEY, displayName TEXT UNIQUE, scim_data JSONB)`);
        pool.query(`CREATE TABLE IF NOT EXISTS group_members (group_id TEXT REFERENCES groups(id) ON DELETE CASCADE, user_id TEXT REFERENCES users(id) ON DELETE CASCADE, PRIMARY KEY (group_id, user_id))`);
    }
});
...
// NEW: Route to fetch and render the groups UI page
app.get('/ui/groups', oidc.ensureAuthenticated(), async (req, res) => {
    try {
        // This query fetches all groups and aggregates their member details into a JSON array.
        const query = `
            SELECT 
                g.id,
                g.displayName,
